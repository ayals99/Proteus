...
bool s1ap:: execute_command(uint8_t* msg, uint16_t len){
  ...
  if(memcmp(msg, "ue_information_request", sizeof("ue_information_request") - 1) == 0) {
    ...
    rrc->rrc_send_ue_information_request_custom(0, integrity, cipher); 
  }
  if(memcmp(msg, "counter_check", sizeof("counter_check") - 1) == 0) {
   ...
   enb::Message message = enb::parseMessage(msg,len);
    ...
    rrc->rrc_send_counter_check_custom(0, integrity, cipher); 
  }
  ...
  if(memcmp(msg, "rrc_security_mode_command", sizeof("rrc_security_mode_command") - 1) == 0) {
    ...
    rrc->rrc_send_security_mode_command_custom(0, integrity, eia, eea); 
  }
  if(memcmp(msg, "rrc_reconf", sizeof("rrc_reconf") - 1) == 0) {
    srsran::console("rrc_reconf\n");
    printf("rrc_reconf!\n");
    enb::Message message = enb::parseMessage(msg,len);
    int integrity = 1;
    int cipher = 1;
    int to_add_mod_list = 0; 
    int set_srb = 0;
    int set_drb = 0;
    int meas_config = 0;
    int replay = 0;
    for(int i=0; i<message.numberOfFields; i++){
      ...
      else if(strcmp(message.messageFields[i].messageFieldName, "set_srb") == 0){
        set_srb =  message.messageFields[i].messageFieldValue;
      }
    ...
    rrc->rrc_send_connection_reconf_custom(0, integrity, cipher, set_srb, set_drb, meas_config, replay); 
  }
