...
with open(os.path.join('considered_inputs_dir', 'message_fields_lte.json'), 'r') as fr:
...
def get_mutated_message(mutated_inp_symbol, input_seq_prefix):
    ...
    print("ERROR Mutation get_mutated_message function")
    exit(1)
        ...
        if not check_symbol_match(response_output_seq[i], reference_output_symbol_sequence[i],
            ...
            if "null_action" in response_output_seq[i]:
                if ("reject" in reference_output_symbol_sequence[i] or
                        "failure" in reference_output_symbol_sequence[i] or
                        "emm_status" in reference_output_symbol_sequence[i]):
                    continue

            if (("reject" in response_output_seq[i] or
                 "failure" in response_output_seq[i] or
...
def get_responsiveness_test(final_reference_state):
    if final_reference_state.startswith("s0"):
        return "enable_s1:", "attach_request:"
    elif final_reference_state.startswith("s1"):
        return "authentication_request:integrity == 0 & cipher == 0 & " \
               "separation_bit == 1 & sqn == 1 & security_header_type == 0", "authentication_response:"

    elif final_reference_state.startswith("s2"):
        return "security_mode_command:integrity == 1 & cipher == 0 & replay == 0 & EIA == 1 & security_header_type == 3", \
        ...
        for i in range(len(self.input_symbol_sequence)):
            ...
            if not check_symbol_match(response_output_seq[i],
                ...
                if ("null_action" in response_output_seq[i] or
                        "reject" in response_output_seq[i] or "failure" in response_output_seq[i] or
                        "emm_status" in response_output_seq[i]):
                    if not ("null_action" in self.attacker_expectation_output_symbol_sequence[i] or
                            ...
                            "failure" in self.attacker_expectation_output_symbol_sequence[i] or "emm_status"
                            in self.attacker_expectation_output_symbol_sequence[i]):
