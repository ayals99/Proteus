...
from adapter_controller import step, pre, initialize_socket, start_core_enb, start_uecontroller, init_core_enb_con, \
    init_ue_con, kill_core, \
    kill_enb, handle_enb_epc_failure, reset_ue
...
device_name = "pixel7_0803_proteus_full_4"
run_mode = 2
...
def create_new_directory_for_current_run(directory):
...
def send_testcase_ota_single_run(test_case: TestCase, concrete_input_sequence, attempt_num):
    ...
    for i in range(len(concrete_input_sequence)):
        time.sleep(1)
        response = step(concrete_input_sequence[i])
    ...
    response = step(responsiveness_input)
...
def send_testcase_ota(test_case: TestCase, concrete_input_sequence):
    ...
    while current_match_count < target_count and attempt_count < 12:
        ...
        if not testcase_pass and not deviating_testcase and responsive:
            ...
            return responsive, respond_output_seq
...
def send_testcase(test_case: TestCase, concrete_input_sequence):
    ...
    if in_db:
        ...
        print("Testing OTA.")
        responsive, response_output_seq = send_testcase_ota(test_case, concrete_input_sequence)
...
def send_tests_single_testcase_dump(testcase_dump_filename,
    ...
    while not (testcase_store.all_done or tested_case_count >= num_tests):
        ...
        if concrete_input_sequence is None:
            break
        ...
        for i in range(len(response_output_seq)):
            if ("attach_request" in response_output_seq[i] and
                    response_output_seq[i] != "attach_request:"):
                good_response = False
        ...
        if "null_action" in response_output_seq[0] or not good_response:
            ...
            while attempts < 10:
                good_response = True
                print("enable_s1 responded with null_action, restarting everything....\n")
                reset_ue()
                time.sleep(1)
                handle_enb_epc_failure()
                full_input_seq, response_output_seq, testcase_pass, deviating_testcase, responsive, in_db, \
