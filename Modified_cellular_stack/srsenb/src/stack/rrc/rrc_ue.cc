...
//                                      bool                         phy_cfg_updated,
//                                      srsran::const_byte_span      nas_pdu)
void rrc::ue::send_connection_reconf_custom(srsran::unique_byte_buffer_t pdu, int integrity, int cipher, int set_srb, int set_drb, int meas_config, int replay)
{
  if (integrity == 1 || cipher == 1)
  {
    if(not ue_security_cfg.is_as_sec_cfg_valid())
  ...
  if (set_srb == 1){
      if (apply_reconf_updates(
            recfg_r8, current_ue_cfg, parent->cfg, ue_cell_list, bearer_list, ue_capabilities, 1)) { 
      parent->logger.error("Generating ConnectionReconfiguration. Aborting...");
      return;
    }
  }
