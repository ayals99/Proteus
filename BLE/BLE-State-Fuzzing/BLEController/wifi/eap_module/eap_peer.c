...
void eap_server_rx(const u8 *data, size_t data_len);
...
static uint8_t use_certificate = 0;
...
int eap_peer_init(const u8 *username, const u8 *password, const u8 *certificate_file)
{
	...
	eap_ctx.eap_config.identity = (u8 *)os_strdup(username);
	eap_ctx.eap_config.identity_len = strlen(eap_ctx.eap_config.identity);

	eap_ctx.eap_config.password = (u8 *)os_strdup(password);
	eap_ctx.eap_config.password_len = strlen(eap_ctx.eap_config.password);
	if (certificate_file)
	{
		eap_ctx.eap_config.cert.ca_cert = os_strdup(certificate_file);
	}
...
void eap_peer_deinit(void)
{
	...
	if (eap_ctx.eap_config.cert.ca_cert)
...
{
	eap_ctx.portEnabled = TRUE;
	eap_ctx.eapRestart = TRUE;
	eap_peer_sm_step(eap_ctx.eap);
}

struct peer_response eap_peer_step(void)
{
	struct peer_response pr;
	struct wpabuf *resp;
	const u8 *key;
	size_t key_len;
	...
	if (eap_ctx.eapResp)
	{
		...
		// Resp must be deallocated by the caler
	}
