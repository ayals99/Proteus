...
def set_monitor_mode(wlan_dev, enable=True, force=False):
    if enable:
        result = subprocess.check_output(['airmon-ng'])
        if wlan_dev + 'mon' in result:
            if force is True:
                print(clr(Color.MAGENTA, 'Disabling monitor mode in interface ' + wlan_dev + 'mon ...'))
                subprocess.check_output(['airmon-ng', 'stop', wlan_dev + 'mon'])
            else:
                return

        result = subprocess.check_output(['airmon-ng', 'start', wlan_dev])

        if (not "monitor mode vif enabled" in result):
            printd(clr(Color.RED,
        ...
        print(clr(Color.MAGENTA, "Airmon set " + wlan_dev + " to monitor mode on " + wlan_dev + 'mon'))
    else:
