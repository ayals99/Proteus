...
static bool send_public_key(struct smp_conn *conn)
{
	...
	if (!ecc_make_key(conn->local_pk, conn->local_sk)) {
        PySys_WriteStdout("IK IK IK ISSUE ISSUE ISSUE IK IK IK \n ");
        return false;
    }
    ...
    int i = 0;
    PySys_WriteStdout("conn->local_pk= ");
    for(i = 0; i < 64; i++){
        PySys_WriteStdout("%x ", conn->local_pk[i]);
    }
    PySys_WriteStdout("\n");
...
static void sc_dhkey_check(struct smp_conn *conn)
{
	...
	memset(r, 0, sizeof(r));
	 int i = 0;
	    //io_cap[1] = 1;
	    PySys_WriteStdout("io_cap= ");
	for(i = 0; i < 3; i++){
        PySys_WriteStdout("%x ", io_cap[i]);
    	}
    	PySys_WriteStdout("\n");
	bt_crypto_f6(conn->smp->crypto, conn->mackey, conn->prnd, conn->rrnd,
				 r, io_cap, local_addr, remote_addr, check.e);

    PySys_WriteStdout("local_addr= ");
    for(i = 0; i < 6; i++){
        PySys_WriteStdout("%x ", local_addr[i]);
    }
    PySys_WriteStdout("\n");
    PySys_WriteStdout("remote_addr= ");
    for(i = 0; i < 6; i++){
        PySys_WriteStdout("%x ", remote_addr[i]);
    }
    PySys_WriteStdout("\n");
    PySys_WriteStdout("smp.c line 241: [IK] Here is where the dh_key check is being send!!\n\n\n\n");
...
static uint8_t sc_passkey_round(struct smp_conn *conn, uint8_t smp_op)
{
	...
	case BT_L2CAP_SMP_PAIRING_RANDOM:
		printf("test IK print here\n");
		PySys_WriteStdout("smp.c line 303: [IK] Here is where the dh_key check is being send!!\n\n\n\n");
		r = ((conn->passkey_notify >> conn->passkey_round) & 0x01);
		r |= 0x80;
		...
		PySys_WriteStdout("smp.c line 338: [IK] Here is where the dh_key check is being send!!\n\n\n\n");
		sc_dhkey_check(conn);
...
static bool verify_random(struct smp_conn *conn, const uint8_t rnd[16])
{
	...
	{
        PySys_WriteStdout("Confirmation values don't match\n");
		//return false;
	}
...
static void distribute_keys(struct smp_conn *conn)
{
	...
	PySys_WriteStdout("conn->local_key_dist & DIST_ENC_KEY=%d\n", conn->local_key_dist & DIST_ENC_KEY);
    ...
    PySys_WriteStdout("IN distribute_keys \n");
	if (conn->local_key_dist & DIST_SIGN)
	{
        PySys_WriteStdout("IN distribute_keys BT_L2CAP_SMP_SIGNING_INFO\n");
...
static void pairing_rsp(struct smp_conn *conn, const void *data, uint16_t len)
{
	...
	int i = 0;
    PySys_WriteStdout("pairing_rsp of cfm= ");
    for(i = 0; i < 16; i++){
        PySys_WriteStdout("%x ", cfm[i]);
    }
	//smp_send(conn, BT_L2CAP_SMP_PAIRING_CONFIRM, cfm, sizeof(cfm));
}
...
{
    PySys_WriteStdout("In smp_send_sm_random smp.c \n ");
    uint8_t rsp[16];
...
{
    PySys_WriteStdout("In smp_send_sm_pair_confirm smp.c \n ");
    uint8_t rsp[16];
    ...
    bt_crypto_c1(conn->smp->crypto, conn->tk, conn->prnd,
                     ...
                     conn->ia, conn->ra_type, conn->ra, rsp);
    int i = 0;
    PySys_WriteStdout("smp_send_sm_pair_confirm of rsp= ");
    for(i = 0; i < 16; i++){
        PySys_WriteStdout("%x ", rsp[i]);
    }
    PySys_WriteStdout("\n");


    
    PySys_WriteStdout("smp_send_sm_pair_confirm of conn->tk= ");
    for(i = 0; i < 16; i++){
        PySys_WriteStdout("%x ", conn->tk[i]);
    }
    PySys_WriteStdout("\n");
...
static uint8_t sc_random(struct smp_conn *conn)
{
	...
	if (conn->out)
	{
		...
		if (memcmp(conn->pcnf, cfm, 16)){
		    PySys_WriteStdout("Something broke IK\n");
            PySys_WriteStdout("conn->smp->crypto=%02X\n", conn->smp->crypto);
            int i =0;
            PySys_WriteStdout("conn->remote_pk= ");
            for(i = 0; i < 64; i++){
                PySys_WriteStdout("%x ", conn->remote_pk[i]);
            }
            PySys_WriteStdout("\n");
            ...
            PySys_WriteStdout("conn->rrnd= ");
            for(i = 0; i < 16; i++){
                PySys_WriteStdout("%x ", conn->rrnd[i]);
            }
            PySys_WriteStdout("\n");
            PySys_WriteStdout("conn->pcnf[i]= ");
            for(i = 0; i < 16; i++){
                PySys_WriteStdout("%x ", conn->pcnf[i]);
            }
            PySys_WriteStdout("\n");
            PySys_WriteStdout("cfm= ");
            for(i = 0; i < 16; i++){
                PySys_WriteStdout("%x ", cfm[i]);
            }
            PySys_WriteStdout("\n");
		    PySys_WriteStdout("conn->pcnf=%02X\n", conn->pcnf);
		    PySys_WriteStdout("cfm=%02X\n", cfm);
            PySys_WriteStdout("Confirm Value Failed But It Doesn't Even Matter!!\n");
	...
	if (conn->out) {
        //PySys_WriteStdout("smp.c line 655: [IK] Here is where the dh_key check is being send!!\n\n\n\n");
        //sc_dhkey_check(conn);
    }
	return 0;
}
...
void smp_pair(void *conn_data, uint8_t io_cap, uint8_t auth_req)
{
	...
	// PySys_WriteStdout("req.init_key_dist=%02X\n", req.init_key_dist);
	// PySys_WriteStdout("req.resp_key_dist=%02X\n", req.resp_key_dist);
...
{
    PySys_WriteStdout("In smp_send_public_key smp.c \n ");
    struct smp_conn *conn = conn_data;
    struct bt_l2cap_smp_pairing_request req;
...
{
    PySys_WriteStdout("In smp_send_dh_key_check smp.c \n ");
    struct smp_conn *conn = conn_data;
    sc_dhkey_check(conn);
    // PySys_WriteStdout("req.init_key_dist=%02X\n", req.init_key_dist);
    // PySys_WriteStdout("req.resp_key_dist=%02X\n", req.resp_key_dist);
...
{
    //uint8_t buf[16];
    //PySys_WriteStdout("In smp_send_sign_info smp.c \n ");
    struct smp_conn *conn = conn_data;
    distribute_keys(conn);
    //if (conn->local_key_dist & DIST_SIGN)
    //{
...
void smp_data(void *conn_data, const void *data, uint16_t len)
{
	...
	case BT_L2CAP_SMP_PAIRING_REQUEST:
        PySys_WriteStdout("\nGot BT_L2CAP_SMP_PAIRING_REQUEST\n");
		pairing_req(conn, data, len);
		break;
	case BT_L2CAP_SMP_PAIRING_RESPONSE:
        PySys_WriteStdout("\nGot BT_L2CAP_SMP_PAIRING_RESPONSE\n");
	...
	case BT_L2CAP_SMP_ENCRYPT_INFO:
		PySys_WriteStdout("Got BT_L2CAP_SMP_ENCRYPT_INFO\n");
		encrypt_info(conn, data, len);
		break;
	case BT_L2CAP_SMP_MASTER_IDENT:
		PySys_WriteStdout("Got BT_L2CAP_SMP_MASTER_IDENT\n");
		master_ident(conn, data, len);
		break;
	case BT_L2CAP_SMP_IDENT_ADDR_INFO:
		// PySys_WriteStdout("Got BT_L2CAP_SMP_IDENT_ADDR_INFO\n");
		ident_addr_info(conn, data, len);
		break;
	case BT_L2CAP_SMP_IDENT_INFO:
		// PySys_WriteStdout("Got BT_L2CAP_SMP_IDENT_INFO\n");
		ident_info(conn, data, len);
		break;
	case BT_L2CAP_SMP_SIGNING_INFO:
		// PySys_WriteStdout("Got BT_L2CAP_SMP_SIGNING_INFO\n");
		signing_info(conn, data, len);
		break;
	case BT_L2CAP_SMP_PUBLIC_KEY:
        PySys_WriteStdout("\nGot BT_L2CAP_SMP_PUBLIC_KEY\n");
